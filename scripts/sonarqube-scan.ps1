...
try {
    ...
    $Null = New-Item -ItemType Directory -Path (Split-Path $JsLcov -Parent) -Force 2>&1
    if (Test-Path -LiteralPath $JsLcov) {
        try {
            Remove-Item -LiteralPath $JsLcov -Force
        } catch {
            $TestResult = 'FAIL(js_lcov_cleanup)'
            $ScanResult = 'NOT_RUN_INVALID_JS_COVERAGE'
            throw "Existing JS coverage report could not be removed before the current run: $JsLcov. Scanner was not run."
        }
    }
    $JsCoverageStartedAt = (Get-Date).ToUniversalTime()
    $VitestExit = 0
    $PreviousPreference = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
    ...
    if ($VitestExit -ne 0) {
        $TestResult = "FAIL(js_exit=$VitestExit)"
        $ScanResult = 'NOT_RUN_JS_COVERAGE_FAILED'
        throw "JS tests or LCOV generation failed (exit $VitestExit). Scanner was not run; see docs/e6/logs/vitest-coverage.log."
    }
    if (-not (Test-Path -LiteralPath $JsLcov -PathType Leaf)) {
        $TestResult = 'FAIL(js_lcov_missing)'
        $ScanResult = 'NOT_RUN_INVALID_JS_COVERAGE'
        throw "JS coverage report was not generated by the current run: $JsLcov. Scanner was not run."
    }
    $JsLcovItem = Get-Item -LiteralPath $JsLcov
    if ($JsLcovItem.Length -eq 0) {
        $TestResult = 'FAIL(js_lcov_empty)'
        $ScanResult = 'NOT_RUN_INVALID_JS_COVERAGE'
        throw "JS coverage report is empty: $JsLcov. Scanner was not run."
    }
    if ($JsLcovItem.LastWriteTimeUtc -lt $JsCoverageStartedAt.AddSeconds(-2)) {
        $TestResult = 'FAIL(js_lcov_stale)'
        $ScanResult = 'NOT_RUN_INVALID_JS_COVERAGE'
        throw "JS coverage report predates the current coverage run: $JsLcov. Scanner was not run."
    }

    Write-Host "Validated fresh JS coverage lcov.info."
    ...
    if ((Get-Item -LiteralPath $JsLcov).Length -eq 0) {
        $TestResult = 'FAIL(js_lcov_empty)'
        $ScanResult = 'NOT_RUN_INVALID_JS_COVERAGE'
        throw "JS coverage report is empty after path normalization: $JsLcov. Scanner was not run."
    }
