...
const MOCK_ROLES = [
  {
    ...
    permissions: [
      { code: 'view_incidents' }, { code: 'create_incidents' },
      { code: 'view_users' }, { code: 'view_dashboard' },
    ],
  },
  {
...
describe('role-permissions-page — integration', () => {
  ...
  describe('utility functions', () => {
    ...
    it('formatModuleLabel handles null and undefined', async () => {
      ...
      expect(formatModuleLabel(undefined)).toBe('');
    });
    ...
    it('getRoleDescription returns fallback for unknown codes', async () => {
      ...
      expect(getRoleDescription(undefined)).toBe('Rol del sistema');
    });
  });
});
