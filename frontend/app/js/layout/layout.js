...
function normalizeMobileSidebar() {
  if (!globalThis.matchMedia('(max-width: 991.98px), (hover: none), (pointer: coarse)').matches) {
    return
  }
...
async function loadNavbarNotifications(forceRefresh = false) {
  ...
  try {
    ...
    console.warn('[SGI] No se pudieron actualizar las notificaciones.', error)
    return null
  }
}
...
function scheduleNotificationFallback() {
  ...
  notificationFallbackTimer = globalThis.setInterval(async () => {
    if (notificationRefreshInFlight || document.visibilityState === 'hidden') {
      return
    }
...
function stopNotificationFallback() {
  if (notificationFallbackTimer) {
    globalThis.clearInterval(notificationFallbackTimer)
  }
...
function getNotificationIconClass(type) {
  ...
  if (value.includes('assigned') || value.includes('asign')) {
    return 'fa-user-check'
  }

  if (value.includes('status') || value.includes('cambio')) {
    return 'fa-sync-alt'
  }

  if (value.includes('closed') || value.includes('resolved') || value.includes('resuelta')) {
    return 'fa-check-circle'
  }

  if (value.includes('comment') || value.includes('comentario')) {
    return 'fa-comment-dots'
  }

  if (value.includes('overdue') || value.includes('vencida')) {
    ...
    sidebarEl.addEventListener('click', e => {
      ...
      if (link.dataset.menuToggle === 'true' || href === '#') {
        ...
        if (navItem) {
          ...
          const siblingItems = [...navItem.parentElement?.children || []]
            .filter(item => item !== navItem && item.classList?.contains('has-treeview'))

          siblingItems.forEach(item => {
            const siblingLink = [...item.children || []].find(child => child.classList?.contains('nav-link'))
...
document.addEventListener('DOMContentLoaded', function initAccessibility() {
  ...
  document.querySelectorAll('[data-card-widget="collapse"]').forEach(btn => {
    if (!btn.getAttribute('aria-label')) {
      btn.setAttribute('aria-label', 'Colapsar sección')
    }
  })
  document.querySelectorAll('[data-card-widget="remove"]').forEach(btn => {
    if (!btn.getAttribute('aria-label')) {
