...
async function getOwnProfilePhotoUrl(user) {
  ...
  if (!key) return null;
  if (key === cachedPhotoKey && cachedPhotoUrl) return cachedPhotoUrl;
  ...
  const requestPromise = requestBlob('/auth/profile/photo', { cache: 'no-store' })
    ...
    .catch(() => null)
    .finally(() => {
      if (pendingPhoto === requestPromise) pendingPhoto = null;
    });
...
globalThis.addEventListener('pagehide', (event) => {
  if (!event.persisted) invalidateOwnProfilePhoto();
});
