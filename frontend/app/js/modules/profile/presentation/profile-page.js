...
export function renderUserData(user) {
  ...
  if (fullNameEl) {
    fullNameEl.textContent = `${firstName} ${lastName}`.trim() || username || 'Usuario SGI'
  }
  ...
  if (memberSinceEl) {
    memberSinceEl.textContent = formatDate(user.created_at || user.createdAt)
  }

  const lastLoginEl = document.getElementById('profileLastLogin')
  if (lastLoginEl) {
    lastLoginEl.textContent = formatDateTime(user.last_login || user.lastLogin)
  }
}
...
export function renderSecurityData(user) {
  ...
  if (has2FA) {
    ...
    if (btnSetup2fa) {
      btnSetup2fa.addEventListener('click', initSetup2FA)
    }
  }
