...
function bindMainMapInteractions() {
  if (!state.map || state.map.__opsBoundInteractions) {
    return
  }
...
function fitZone(zone) {
  ...
  if (!features.length || !globalThis.maplibregl || !state.map) {
    return
  }
...
function setHoveredFeature(featureId) {
  if (!state.map || state.hoveredFeatureId === featureId) {
    return
  }
...
function clearHoveredFeature() {
  if (!state.map || state.hoveredFeatureId === null) {
    return
  }
