...
export function getPostAuthPage(user) {
  ...
  if (userHasPermission(user, 'incidents.create')) {
...
export function switchView(viewName) {
  ...
  Object.entries(viewMap).forEach(([id, name]) => {
    ...
    if (el) {
      el.classList.toggle('d-none', viewName !== name)
    }
  })
  ...
  if (viewName !== 'register') {
    hideAlert(document.getElementById('register-alert'))
  }

  if (viewName !== 'profile') {
    hideAlert(document.getElementById('profile-alert'))
  }

  if (viewName !== 'two-factor') {
    hideAlert(document.getElementById('two-factor-alert'))
  }

  if (viewName !== 'setup-2fa') {
    hideAlert(document.getElementById('setup-2fa-alert'))
  }

  if (viewName !== 'forgot-password') {
    hideAlert(document.getElementById('forgot-password-alert'))
  }
}
...
export function initAuthPage() { // NOSONAR - Inherently complex multi-view auth page with form handling, Google OAuth, 2FA setup, and password recovery flows
  ...
  function bindEvents() {
    ...
    if (el.twoFactorForm) {
      el.twoFactorForm.addEventListener('submit', handleTwoFactorSubmit)
    }

    if (el.setupTwoFactorForm) {
      el.setupTwoFactorForm.addEventListener('submit', handleSetupTwoFactorSubmit)
    }

    if (el.forgotPasswordForm) {
      el.forgotPasswordForm.addEventListener('submit', handleForgotPasswordSubmit)
    }

    if (el.resetCodeForm) {
      el.resetCodeForm.addEventListener('submit', handleResetCodeSubmit)
    }

    if (el.resetPasswordForm) {
      el.resetPasswordForm.addEventListener('submit', handleResetPasswordSubmit)
    }

    if (el.openLoginBtn) {
      el.openLoginBtn.addEventListener('click', event => {
        event.preventDefault()
        switchView('login')
      })
    }

    if (el.openRegisterBtn) {
      el.openRegisterBtn.addEventListener('click', event => {
        event.preventDefault()
        switchView('register')
      })
    }

    if (el.emailVerificationLoginBtn) {
      el.emailVerificationLoginBtn.addEventListener('click', returnToLogin)
    }

    if (el.emailVerificationResendBtn) {
      el.emailVerificationResendBtn.addEventListener('click', resendPendingVerification)
    }

    if (el.backToLoginBtn) {
      el.backToLoginBtn.addEventListener('click', () => switchView('login'))
    }

    if (el.openForgotPasswordBtn) {
      el.openForgotPasswordBtn.addEventListener('click', event => {
        event.preventDefault()
        beginPasswordRecovery()
      })
    }

    if (el.backToLoginFromResetBtn) {
      el.backToLoginFromResetBtn.addEventListener('click', cancelPasswordRecovery)
    }

    if (el.backToLoginFromResetBtn2) {
      el.backToLoginFromResetBtn2.addEventListener('click', cancelPasswordRecovery)
    }

    if (el.backToLoginFromResetBtn3) {
      el.backToLoginFromResetBtn3.addEventListener('click', cancelPasswordRecovery)
    }

    if (el.resendResetCodeBtn) {
      el.resendResetCodeBtn.addEventListener('click', resendPasswordResetCode)
    }

    const backToLoginFrom2FaBtn = document.getElementById('back-to-login-from-2fa-btn')
    if (backToLoginFrom2FaBtn) {
      backToLoginFrom2FaBtn.addEventListener('click', () => {
        tempTwoFactorToken = null
        switchView('login')
      })
    }

    if (el.googleRegisterBtn) {
      el.googleRegisterBtn.addEventListener('click', e => handleGoogleAuth(e, 'register'))
    }

    if (el.googleLoginBtn) {
      el.googleLoginBtn.addEventListener('click', e => handleGoogleAuth(e, 'login'))
    }
  ...
  function beginPasswordRecovery() {
    ...
    recoveryCode = ''
    if (el.forgotEmailInput) {
      el.forgotEmailInput.value = recoveryEmail
    }
    ...
    if (el.resetPasswordInput) {
      el.resetPasswordInput.value = ''
    }

    if (el.resetPasswordConfirmInput) {
  ...
  async function handleForgotPasswordSubmit(event) {
    event.preventDefault()
    if (!validateFormFrontend(el.forgotPasswordForm)) {
      return
    }
  ...
  async function resendPasswordResetCode() {
    ...
    if (el.resendResetCodeBtn) {
      el.resendResetCodeBtn.disabled = true
    }
    ...
    try {
      ...
      if (el.resendResetCodeBtn) {
        el.resendResetCodeBtn.disabled = false
      }
    }
  }
  ...
  async function handleResetCodeSubmit(event) {
    event.preventDefault()
    if (!validateFormFrontend(el.resetCodeForm)) {
      return
    }
  ...
  async function handleResetPasswordSubmit(event) {
    event.preventDefault()
    if (!validateFormFrontend(el.resetPasswordForm)) {
      return
    }
    ...
    try {
      ...
      switchView('login')
      if (el.loginEmailInput && recoveryEmail) {
        el.loginEmailInput.value = recoveryEmail
      }
  ...
  function showEmailVerificationScreen({ email, eyebrow, title, copy, help, warning = '' }) {
    ...
    if (el.emailVerificationEyebrow) {
      el.emailVerificationEyebrow.textContent = eyebrow
    }

    if (el.emailVerificationTitle) {
      el.emailVerificationTitle.textContent = title
    }

    if (el.emailVerificationCopy) {
      el.emailVerificationCopy.textContent = copy
    }

    if (el.emailVerificationEmail) {
      el.emailVerificationEmail.textContent = email
    }

    if (el.emailVerificationHelp) {
      el.emailVerificationHelp.textContent = help
    }
  ...
  async function resendPendingVerification() {
    if (!pendingVerificationEmail || !el.emailVerificationResendBtn) {
      return
    }
  ...
  function switchView(viewName) {
    ...
    if (el.loginView) {
      el.loginView.classList.toggle('d-none', viewName !== 'login')
    }

    if (el.registerView) {
      el.registerView.classList.toggle('d-none', viewName !== 'register')
    }

    if (el.emailVerificationView) {
      el.emailVerificationView.classList.toggle('d-none', viewName !== 'email-verification')
    }

    if (el.profileView) {
      el.profileView.classList.toggle('d-none', viewName !== 'profile')
    }

    if (el.twoFactorView) {
      el.twoFactorView.classList.toggle('d-none', viewName !== 'two-factor')
    }

    if (el.setupTwoFactorView) {
      el.setupTwoFactorView.classList.toggle('d-none', viewName !== 'setup-2fa')
    }

    if (el.forgotPasswordView) {
      el.forgotPasswordView.classList.toggle('d-none', viewName !== 'forgot-password')
    }

    if (viewName !== 'login' && el.loginAlert) {
      hideAlert(el.loginAlert)
    }

    if (viewName !== 'register' && el.registerAlert) {
      hideAlert(el.registerAlert)
    }

    if (viewName !== 'profile' && el.profileAlert) {
      hideAlert(el.profileAlert)
    }

    if (viewName !== 'two-factor' && el.twoFactorAlert) {
      hideAlert(el.twoFactorAlert)
    }

    if (viewName !== 'setup-2fa' && el.setupTwoFactorAlert) {
      hideAlert(el.setupTwoFactorAlert)
    }

    if (viewName !== 'forgot-password' && el.forgotPasswordAlert) {
      hideAlert(el.forgotPasswordAlert)
    }
  }
  ...
  function setLoading(scope, isLoading) {
    ...
    if (map.spinner) {
      map.spinner.classList.toggle('d-none', !isLoading)
    }
  ...
  async function initMandatorySetup2FA() {
    try {
      if (!el.setupTwoFactorQrContainer || !el.setupTwoFactorForm) {
        return
      }
      ...
      showAlert(el.setupTwoFactorAlert, error.message || 'No se pudo inicializar la autenticación en dos pasos.', 'danger')
    }
  }
  ...
  function ensureQrCodeLibrary() {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.addEventListener('error', () => reject(new Error('No se pudo cargar el generador de código QR.')))
